...
function registrarGrupo($nombreg, $especialidad, $turno, $semestre, $generacion){
    ...
    $query = "SELECT nombre FROM Grupo WHERE nombre='$nombreg'";
    $nombreg2 = $conn->query($query);
    $query = "SELECT especialidad FROM Grupo WHERE especialidad='$especialidad'";
    $especialidad2 = $conn->query($query);
    $query = "SELECT turno FROM Grupo WHERE turno='$turno'";
    $turno2 = $conn->query($query);
    $query = "SELECT semestre FROM Grupo WHERE semestre='$semestre'";
    $semestre2 = $conn->query($query);
    $query = "SELECT generacion FROM Grupo WHERE generacion='$generacion'";
    $generacion2 = $conn->query($query);
    if($nombreg2==$nombreg && $especialidad2==$especialidad && $turno2==$turno && $semestre2==$semestre && $generacion2==$generacion){
        die("Este grupo ya existe en la BD " .$statement->errno.": ".$statement->error);
    }
